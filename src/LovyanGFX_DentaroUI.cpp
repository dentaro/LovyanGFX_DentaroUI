  ...
  for(int i= 0; i < uiBoxes[uiID].row; i++)
    {
      for(int j= 0; j < uiBoxes[uiID].col; j++)
      {
        touch_btn_list[_startId + j*uiBoxes[uiID].row + i]->setAvailableF(_available);//ベータ版は基本true
      }
    }
}
...
void LovyanGFX_DentaroUI::setAvailableF(int uiID, int _btnID, bool _available){
  int _startId = uiBoxes[uiID].b_sNo;//スタート時のボタンIDをセット
  touch_btn_list[_startId + _btnID]->setAvailableF(false);
}
...
// void LovyanGFX_DentaroUI::drawBtns(int _uiID, LovyanGFX* _lgfx, LGFX_Sprite& _uiSprite, int _uiSprite_x, int _uiSprite_y){
//   toggle_mode = false;
...
lgfx::v1::touch_point_t LovyanGFX_DentaroUI::getSliderPos(int uiID, int _btnNo){
  ...
  if(touch_btn_list[_btnID]->get_xy_mode() == XY_VAL){
    getTouchPoint(touch_btn_list[_btnID]->getSliderValx(), touch_btn_list[_btnID]->getSliderValy());
  }
  return retPos;
}
