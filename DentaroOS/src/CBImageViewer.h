...
//    btnDrawer.setText(2, "QRcode");
    ...
    lcd.drawString("ImageViewer : " + mi->path, 10, 10, &fonts::Font2);
...
private:
  ...
  void drawImage()
  {
    ...
    if (ext == "bmp") {
      lcd.drawBmpFile(mi->getFS(), mi->path.c_str(), 0, 26);
    } else
    if (ext == "png") {
      lcd.drawPngFile(mi->getFS(), mi->path.c_str(), 0, 26);
    } else
    if (ext == "jpg" || ext == "jpeg") {
      lcd.drawJpgFile(mi->getFS(), mi->path.c_str(), 0, 26);
    } else
    if (ext == "mag") {
      drawMagFile(lcd, mi->getFS(), mi->path.c_str(), 0, 26);
    }
  }
  ...
  void drawMagFile(LGFX &lcd, FS& fs, const char* path, int dst_x, int dst_y) {
  ...
  //  uint16_t *linebuf= (uint16_t *)heap_caps_malloc(320, MALLOC_CAP_DMA);
    ...
    for (int y = 0; y < height; ++y) {
      if (0 != dest && (0 == (y & 1) || (y + 1 == height))) {
        ...
        // lcd.pushColors(linebuf, x, false);//廃止
        lcd.pushPixels(linebuf, x, false);
        if (y + 1 == height) break;
        if ((y & 15) == 0) {
          destdiff = dest;
          dest = 0;
